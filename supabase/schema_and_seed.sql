-- =============================================================================
-- TRACIFY / VASPTRACE — SUPABASE DATABASE SCHEMA & SEED DATA
...
CREATE POLICY "cases_insert" ON public.cases FOR INSERT TO authenticated WITH CHECK (created_by IS NULL OR created_by = auth.uid());
...
CREATE POLICY "cases_update" ON public.cases FOR UPDATE TO authenticated
  ...
  WITH CHECK (created_by IS NULL OR created_by = auth.uid() OR public.has_role(auth.uid(), 'admin'));
...
CREATE POLICY "investigations_insert" ON public.investigations FOR INSERT TO authenticated WITH CHECK (created_by IS NULL OR created_by = auth.uid());
...
CREATE POLICY "findings_insert" ON public.findings FOR INSERT TO authenticated WITH CHECK (created_by IS NULL OR created_by = auth.uid());
...
CREATE INDEX IF NOT EXISTS findings_investigation_idx ON public.findings(investigation_id);
...
CREATE POLICY "evidence_insert" ON public.evidence FOR INSERT TO authenticated WITH CHECK (added_by IS NULL OR added_by = auth.uid());
...
CREATE POLICY "evidence_update" ON public.evidence FOR UPDATE TO authenticated
  ...
  WITH CHECK (added_by IS NULL OR added_by = auth.uid() OR public.has_role(auth.uid(), 'admin'));
...
CREATE POLICY "reports_insert" ON public.reports FOR INSERT TO authenticated WITH CHECK (created_by IS NULL OR created_by = auth.uid());
...
-- ===== 10. FUNCTION EXECUTION PERMISSIONS =====
...
INSERT INTO public.reports (id, report_ref, case_id, investigation_id, title, status, sections, notes, created_at)
VALUES
 ('44444444-4444-4444-8444-444444444401', 'RPT-0231', '11111111-1111-4111-8111-111111111101', '22222222-2222-4222-8222-222222222201',
  ...
  now() - interval '5 days'),
 ('44444444-4444-4444-8444-444444444402', 'RPT-0198', '11111111-1111-4111-8111-111111111104', '22222222-2222-4222-8222-222222222205',
